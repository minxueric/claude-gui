...
export default function ChatPage() {
  ...
  const { state, sendInput, respondPermission, respondAskUserQuestion, respondPlanExit, interrupt, setMode, setTurns } = useChatStream(chatId, permissionMode);
                ...
                elements.push(
                  <div key={`g-${start}`} ref={(el) => (turnRefs.current[start] = el)} data-turn-index={start}>
                    <AssistantTurnGroup
                      ...
                      planExitPending={isLast ? state.planExitPending : undefined}
                      onPlanExit={isLast ? respondPlanExit : undefined}
                    />
                  </div>
                );
                buffer = [];
